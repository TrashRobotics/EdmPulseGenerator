...
#define LEVEL_2_PULSE_TIME 4.0f  // до 3 мкс прибавляться будет по +-0.1 мкс. После 3 мкс по +-1 мкс
...
void loop(){
  ...
  voltage = voltageFilter(instantVoltage, 0.03);  // усредненные значения тока и напряжения
  current = currentFilter(instantCurrent, 0.03);

  //if ((instantCurrent >= maxPulseCurrent) || (voltage < maxVoltage*0.4)){   // если короткое замыкание
